...
export default class Canvas {
  ...
  _init() {
    ...
    this.canvas.click(function (e) {
      ...
      if (
        !currentTool ||
        currentTool instanceof ConnectionTool ||
    ...
    function dragEnd(event) {
      ...
      for (let i = 0; i < allFigures.length; i++) {
        ...
        if (figure instanceof Connection) {
          ...
          if (
            (figureBounds.x >= bounds.x && figureBounds.x <= bounds.x2) ||
            (figureBounds.x2 >= bounds.x && figureBounds.x2 <= bounds.x2)
          ) {
            if (
              (figureBounds.y >= bounds.y && figureBounds.y <= bounds.y2) ||
              (figureBounds.y2 >= bounds.y && figureBounds.y2 <= bounds.y2)
            ) {
